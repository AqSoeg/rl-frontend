...
const OfflineDatabase = ({ datasets, fetchDatasets }) => {
    ...
    const columns = [
        ...
        { title: '离线数据中的状态信息', dataIndex: 'DATA_STATE', key: 'DATA_STATE', onHeaderCell: () => ({ style: { whiteSpace: 'nowrap' } }) },
        { title: '离线数据中的动作信息描述', dataIndex: 'DATA_ACTION', key: 'DATA_ACTION', onHeaderCell: () => ({ style: { whiteSpace: 'nowrap' } }) },
        { title: '数据库文件路径', dataIndex: 'DATASET_PATH', key: 'DATASET_PATH', onHeaderCell: () => ({ style: { whiteSpace: 'nowrap' } }) },
        { title: '创建时间', dataIndex: 'CREATE_TIME', key: 'CREATE_TIME', render: time => new Date(time).toLocaleString(), onHeaderCell: () => ({ style: { whiteSpace: 'nowrap' } }) },
        {
            title: '操作',
            key: 'action',
            width:150,
